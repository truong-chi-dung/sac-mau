      ...
      <tr>
        ...
        <td class="sortCol"><div>Tên bài viết<span></span></div></td>
		 <?php if($config_images=='true'){ ?> 
			<td width="100"><div>Hình ảnh</div></td>
		  <?php } ?>
    ...
    <tbody>
       ...
       <td>
        ...
        <td class="title_name_data" align="center">
		 ...
		 <?php if($config_images=='true'){ ?> 
        <td align="center">
            <a href="index.php?com=baiviet&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id_sub=<?=$items[$i]['id_sub']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><img src="<?=_upload_baiviet.$items[$i]['thumb']?>" class="img_max" width="80" alt=""> </a>
        </td>
		<?php } ?>
        <?php if($config_noibat=='true'){ ?> 
        <td align="center">
