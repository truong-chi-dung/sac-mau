...
function save_item(){
	...
	if($id){
		$id =  themdau($_POST['id']);
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_thuoctinh,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 295, 195, _upload_thuoctinh,$file_name,1);		
			$d->setTable('thuoctinh');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_thuoctinh.$row['photo']);	
				delete_file(_upload_thuoctinh.$row['thumb']);				
			}
		}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb from #_thuoctinh where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
